describe 'PUT update shape', type: :request do
  let!(:cut_materials) { prepare_cut_and_materials }
  let!(:client) { create :client }
  let!(:another_client) { create :client }
  let!(:detail) { Details::AddToList.call(detail: client.details.new, detail_data: FixtureData.circle_with_circle_holes_center_center.merge(material: CutMaterial.first.id)).detail }
  let!(:another_detail) { Details::AddToList.call(detail: another_client.details.new, detail_data: FixtureData.circle_with_circle_holes_center_center1.merge(material: CutMaterial.first.id)).detail }

  context 'when user not authorized' do
    before { put("/constructor/shape/#{detail.id}", nil, nil) }
  ...
  context 'when client get not own detail' do
    before do
      login_as(client)
      put("/constructor/shape/#{another_detail.id}", nil, nil)
    end
  ...
  context 'name already exists' do
    let!(:detail1) { Details::AddToList.call(detail: client.details.new, detail_data: FixtureData.rectangle_with_rectangle_center_holes.merge(material: CutMaterial.first.id)).detail }
    let!(:params) { {shape: FixtureData.rectangle_with_rectangle_center_holes.merge(material: CutMaterial.third.id)} }

    before do
      login_as(client)
      put("/constructor/shape/#{detail.id}", nil, params)
    end
  ...
  context 'when client get own detail' do
    ...
    context 'and send valid json' do
      ...
      it 'should create detail' do
        expect(client.details.count).to eq(1)
